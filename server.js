...
app.get("/api/update/:app", async (req, res) => {
    ...
    let release
    if (version) release = await getCachedReleaseByTag(appName, version)
    else { const releases = await getCachedReleases(appName); release = releases && releases.length > 0 ? releases[0] : null }
    ...
    res.json({ version: release.tag_name, published_at: release.published_at, release_notes: release.body, assets })
})
...
app.get("/download/:app", async (req, res) => {
    ...
    let asset
    if (assetName) { asset = release.assets.find(a => a.name === assetName); if (!asset) return res.status(404).send("Asset not found") }
    else asset = selectAssetForPlatform(release.assets, userAgent, platformHint)
    try {
