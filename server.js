...
async function getSignatureForAsset(appName, binaryAsset, release) {
    const version = release.tag_name;
    const sigAsset = release.assets.find(a => a.name === binaryAsset.name + ".sig");
    if (!sigAsset) {
        console.warn(`[Signature Missing] No .sig file for ${binaryAsset.name} (${version})`);
        return null;
    }
    const sigPath = await ensureAssetCached(appName, sigAsset, version);
    try {
        return await fs.readFile(sigPath, "utf8");
    } catch (err) {
        console.error(`[Signature Read Error] ${sigPath}: ${err.message}`);
        return null;
    }
}

function getAssetForTarget(assets, target) {
    ...
    function scoreAsset(asset) {
        ...
        if (
            os === "windows" &&
            (name.includes("windows") || name.includes("win") || name.endsWith(".exe") || name.endsWith(".msi"))
        )
            score += 10;
        if (
            os === "darwin" &&
            (name.includes("mac") || name.includes("darwin") || name.endsWith(".dmg") || name.endsWith(".app"))
        )
            score += 10;
        if (
            os === "linux" &&
            ...
            score += 10;
        if (arch === "x86_64" && (name.includes("x64") || name.includes("amd64") || name.includes("x86_64")))
            score += 5;
        if (arch === "aarch64" && (name.includes("arm64") || name.includes("aarch64"))) score += 5;
        if (arch === "i686" && (name.includes("x86") || name.includes("i686"))) score += 5;
        return score;
    }
    const scored = assets.map(asset => ({ asset, score: scoreAsset(asset) }));
    scored.sort((a, b) => b.score - a.score);
    console.log(`[Target Match] target=${target}`);
...
app.get("/api/update/tauri/:app", async (req, res) => {
    const appName = req.params.app.toLowerCase();
    ...
    const asset = getAssetForTarget(release.assets, target);
    if (!asset) {
        return res.status(404).json({ error: `No asset found for target ${target}` });
    }

    const signature = await getSignatureForAsset(appName, asset, release);
    if (!signature) {
        return res.status(500).json({ error: "Signature missing for update asset" });
    }

    const downloadUrl = `${req.protocol}://${req.get("host")}/download/${appName}?version=${encodeURIComponent(release.tag_name)}&asset=${encodeURIComponent(asset.name)}`;
...
function selectAssetForPlatform(assets, userAgent, platformHint) {
    ...
    function scoreAsset(asset) {
        ...
        if (os === "linux" && (name.endsWith(".appimage") || name.endsWith(".deb") || name.endsWith(".rpm")))
            score += 8;
        return score;
    }
    const scored = assets.map(asset => ({ asset, score: scoreAsset(asset) }));
